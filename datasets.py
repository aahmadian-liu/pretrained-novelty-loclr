...
# paths to datasets on disk (CUB and PCAM might need manual downloading)
data_path={
    'cub':'',
    'flowers':'',
    'food':'',
...
# transformation applied to all images (based on DINO) given mean and std
...
def get_dataset_model(dataset_name:str,classes_in:list,classes_out:list,model_name:str,finetuned_model:str,rand_seed,has_out_train=False):
    ...
    print("Backbone model intialized:",model_name)
    ...
    if dataset_name=='cub':
        ...
        # train and test swapped for flowers
        if len(classes_out) == 0:
        ...
        if classes_in is not None and classes_in[0]>0:
            print("class indexes for PCAM ignored")
